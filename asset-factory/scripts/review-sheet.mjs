...
// confirms they exist and are readable, and assembles one contact-sheet PNG plus
// a criteria checklist for local visual review. Run on a normally networked
// machine — this is the step the sandboxed Claude session cannot do itself.
//
// Usage: node review-sheet.mjs <screen-slug>
...
if (!slug) {
  console.error('Usage: node review-sheet.mjs <screen-slug>')
  process.exit(1)
}
...
for (const sub of ['raw', 'assets', 'previews', 'references']) {
...
function latestFinalAttempt(asset) {
  ...
  // last attempt with a raw_url is the most recent real render for this asset.
...
if (masterUrl) {
  ...
  console.error('No master_reference_higgsfield_media.url in manifest and no local master-reference.png. Aborting.')
  process.exit(1)
}

// 2. Each generated asset's latest final + optional cutout
const assetFiles = {} // asset.id -> { rawPath, cutoutPath, meta }
...
function svgLabel(text, width, height = LABEL_H) {
  const esc = text.replace(/&/g, '&amp;').replace(/</g, '&lt;')
  return Buffer.from(`<svg width="${width}" height="${height}">
    <rect width="100%" height="100%" fill="#1c2b27"/>
...
const rows = [] // each row: [{label, cell}]

// Row 1: master reference (full) + hero landscape final
rows.push([
  { label: 'Master dashboard reference (style/composition source — never a layer)', cell: await cellFromFile(masterPath) },
  assetFiles['hero-landscape'] ? { label: 'Hero Landscape — final (keeps background)', cell: await cellFromFile(assetFiles['hero-landscape'].rawPath) } : null
])

// Row 2: EQ final on white + EQ cutout on dark
...
// Row 3: Family Quest final on white + cutout on dark
...
// Row 4: fixed-region 100% crops for edge inspection.
// These are representative regions, not content-aware detection — inspect the
// full raw/ files yourself too if a specific spot needs a closer look.
const cropRow = []
if (assetFiles['eq-greeting-companion']) {
  const eq = assetFiles['eq-greeting-companion']
  ...
  if (eq.cutoutPath) {
    cropRow.push({ label: 'EQ cutout — left-side edge crop on DARK (arm/leg silhouette)', cell: await cropFraction(eq.cutoutPath, { x0: 0, y0: 0.3, x1: 0.35, y1: 0.9 }).then(async c => ({ ...(await onDarkBuf(c)), })) })
    cropRow.push({ label: 'EQ cutout — right-side edge crop on DARK (arm/leg silhouette)', cell: await cropFraction(eq.cutoutPath, { x0: 0.65, y0: 0.3, x1: 1, y1: 0.9 }).then(async c => ({ ...(await onDarkBuf(c)), })) })
  }
}
if (assetFiles['family-quest-island']) {
  const fq = assetFiles['family-quest-island']
  ...
  if (fq.cutoutPath) {
    cropRow.push({ label: 'Island cutout — underside taper edge on DARK', cell: await cropFraction(fq.cutoutPath, { x0: 0.2, y0: 0.55, x1: 0.8, y1: 1 }).then(async c => ({ ...(await onDarkBuf(c)), })) })
  }
}
...
for (const row of twoColRows) {
  ...
  for (const cell of row) {
    if (cell) {
      composites.push({ input: svgLabel(cell.label, colWidths[0]), left: x, top: cursorY })
...
// crop row: wrap into a grid of up to 3 per line
const cropCols = 3
...
Open \`previews/review-sheet.png\` at full size (and the individual files in
\`raw/\` at 100% zoom for anything the fixed crops don't cover) and check every
box below. Mark PASS or FAIL with a short note. One failing box on an asset
means that asset fails — use the single remaining stop-loss attempt for it,
preserving the approved composition rather than reinterpreting it creatively.
...
- [ ] Composition supports the approved dashboard hero (open space where the character sits, sky/hills/mosque arrangement matches)
- [ ] No text, no letters, no UI artifacts baked into the image
- [ ] No unintended extra objects (no people, no animals, nothing not asked for)
- [ ] Crop-safe for a responsive mobile hero band (mosque + open space both survive a reasonable vertical crop)
...
- [ ] Art style matches the approved dashboard (same palette, same lighting quality)

**Verdict:** PASS / FAIL — notes: ____________________

## Next step

- All three PASS → run \`npm run promote --workspace scripts -- ${manifest.slug}\` (or \`node scripts/promote.mjs ${manifest.slug}\`) after filling in \`screens/${manifest.slug}/review-decision.json\`.
- Any FAIL → do not promote. Regenerate only the failing asset (1 stop-loss attempt remains per the manifest), keeping the same approved composition, then re-run the review sheet.
`
fs.writeFileSync(checklistPath, checklist)
console.log(`Checklist written to:   ${checklistPath}`)

// starter decision file so Faizal just edits pass/fail values
const decisionPath = path.join(dir, 'review-decision.json')
if (!fs.existsSync(decisionPath)) {
  fs.writeFileSync(decisionPath, JSON.stringify({
    'eq-greeting-companion': 'pending',
    'hero-landscape': 'pending',
    'family-quest-island': 'pending'
  }, null, 2))
  console.log(`Decision file written to: ${decisionPath}  (set each to "approve" or "reject" after inspection)`)
}

console.log('\nDone. Nothing was auto-approved.')
